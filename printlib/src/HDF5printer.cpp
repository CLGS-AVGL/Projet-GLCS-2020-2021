...
void write_all(const Distributed2DField &data, const Configuration &config) {
  // test MPI_init call
  int init_flag;
  ...
  std::string file_name = config.output_filename();
  hid_t h5file;
  ...
  // the total matrix size, without ghost cells
  const hsize_t block[2] = {data.global_shape()[DY], data.global_shape()[DX]};

  // each process has its own block_size
  const hsize_t block_size[2] = {data.noghost_view().extent(DY),
                                 data.noghost_view().extent(DX)};
  ...
  if (dataset_id < 0) {
    std::cerr << "error in H5Dcreate, in hdf5printer.cpp, exit." << std::endl;
    exit(-1);
  }
  ...
  start_w[0] = data.distribution().coord(DY) *
               data.noghost_view().extent(DY); // how to find the begin-height
  start_w[1] = data.distribution().coord(DX) *
               data.noghost_view().extent(DX); // how to find the begin-width
  ...
  // stack overflow risk but... malloc abort...
  double ptr[data.noghost_view().extent(DY) * data.noghost_view().extent(DX) *
             sizeof(double)];

  // why is it needed !!!
  for (int i = data.noghost_view().extent(DY); i >= 0; i--)
    for (int j = 0; j < data.noghost_view().extent(DX); j++)
      ptr[i * data.noghost_view().extent(DX) + j] = data.noghost_view(i, j);
...
// class printer HDF5
void HDF5printer::simulation_updated(const Distributed2DField &data,
                                     const Configuration &config) {
  bool its_time_to_write_all = false;
  double nb_dt = data.time() / config.delta_t();
  int nb = static_cast<int>(nb_dt);
  if (nb % config.backup_interval() == 0) {
    its_time_to_write_all = true;
  }

  if (its_time_to_write_all) {
