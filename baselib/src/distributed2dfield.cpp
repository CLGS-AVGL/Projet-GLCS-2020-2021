...
Distributed2DField::Distributed2DField(MPI_Comm comm, Shape2D dist_shape,
                                       Shape2D global_shape,
                                       Shape2D ghost_sizes,
                                       std::array<double, 2> delta_space)
    : m_distribution(comm, dist_shape), m_data(0), m_delta_space(delta_space),
      m_global_shape(global_shape), m_reducedValue(0.0) {
  for (int ii = 0; ii < m_data.size(); ++ii) {
    m_data[ii] = ii;
  }
  // the shape of the local bloc without ghost
  const Shape2D noghost_shape = {global_shape[DY] / dist_shape[DY],
                                 global_shape[DX] / dist_shape[DX]};

  // the shape of the full local bloc, with ghost added
  const Shape2D full_shape = {noghost_shape[DY] + 2 * ghost_sizes[DY],
                              noghost_shape[DX] + 2 * ghost_sizes[DX]};

  m_data.resize(full_shape[DY] * full_shape[DX]);
  mdspan<double, dynamic_extent, dynamic_extent> data2d(
      m_data.data(), full_shape[DY], full_shape[DX]);

  // a view of the full local bloc, including the whole data with ghosts
  m_full_view = subspan(data2d, pair<int, int>{0, full_shape[DY]},
  ...
  m_noghost_view = subspan(
      data2d, pair<int, int>{ghost_sizes[DY], full_shape[DY] - ghost_sizes[DY]},
  ...
  m_ghost_views[DOWN] = subspan(
      data2d, pair<int, int>{0, ghost_sizes[DY]},
      pair<int, int>{ghost_sizes[DX], full_shape[DX] - ghost_sizes[DX]});

  m_ghost_views[UP] = subspan(
      data2d,
      pair<int, int>{ghost_sizes[DY] + noghost_shape[DY], full_shape[DY]},
      pair<int, int>{ghost_sizes[DX], full_shape[DX] - ghost_sizes[DX]});

  m_ghost_views[LEFT] = subspan(
      data2d, pair<int, int>{ghost_sizes[DY], full_shape[DY] - ghost_sizes[DY]},
      pair<int, int>{0, ghost_sizes[DX]});

  m_ghost_views[RIGHT] = subspan(
      data2d, pair<int, int>{ghost_sizes[DY], full_shape[DY] - ghost_sizes[DY]},
      pair<int, int>{ghost_sizes[DX] + noghost_shape[DX], full_shape[DX]});

  // A type representing a column ghost (i.e. the left or right one)
  MPI_Type_vector(noghost_shape[DY], ghost_sizes[DX], full_shape[DX],
                  MPI_DOUBLE, &m_ghost_col);
  MPI_Type_commit(&m_ghost_col);

  // A type representing a row ghost (i.e. the top or bottom one)
  MPI_Type_vector(ghost_sizes[DY], noghost_shape[DX], full_shape[DX],
                  MPI_DOUBLE, &m_ghost_row);
...
void Distributed2DField::sync_ghosts() {
  ...
  // the coordinate of the last block of the same size as the ghost in the
  // no-ghost zone
