...
public:
   ...
   * @param dist_shape the shape of the array distribution, i.e. the number of
   * processes in each dimension
   * @pre dist_shape[Dim::Y] * dist_shape[Dim::Y] must be equal to
   * MPI_Comm_size(comm)
   * @param global_shape the shape of the global distributed , i.e. the number
   ...
   */
  Distributed2DField(MPI_Comm comm, Shape2D dist_shape, Shape2D global_shape,
                     Shape2D ghost_sizes, std::array<double, 2> delta_space);
   ...
   */
  View ghost_view(Direction2D id) {
    return m_ghost_views[static_cast<int>(id)];
  }
   ...
   */
  const View ghost_view(Direction2D id) const {
    return m_ghost_views[static_cast<int>(id)];
  }
   ...
   * @return a modifiable view of a ghost
   */
  double &ghost_view(Direction2D id, int yy, int xx) {
    return ghost_view(id)(yy, xx);
  }
   ...
   * @return a constant view of a ghost
   */
  double ghost_view(Direction2D id, int yy, int xx) const {
    return ghost_view(id)(yy, xx);
  }
