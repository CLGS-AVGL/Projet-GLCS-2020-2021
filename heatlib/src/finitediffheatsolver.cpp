...
void FinitediffHeatSolver::iter(const Distributed2DField &cur,
                                Distributed2DField &next) const {
  ...
  for (int yy = 0; yy < cur_noghost.extent(DY); ++yy) {
    for (int xx = 0; xx < cur_noghost.extent(DX); ++xx) {
      next_noghost(yy, xx) =
          cur_noghost(yy, xx) +
          m_delta_t * ((cur_noghost(yy - 1, xx) - cur_noghost(yy, xx) * 2 +
                        cur_noghost(yy + 1, xx)) /
                           cur.delta(DY) +
                       (cur_noghost(yy, xx - 1) - cur_noghost(yy, xx) * 2 +
                        cur_noghost(yy, xx + 1)) /
                           cur.delta(DX));
    }
  }
