...
void FileConditions::initial_condition(Distributed2DField &data,
  ...
  // except the boundary condition on the left if our block is at the boundary
  // itself
  ...
  if (h5file < 0) {
    std::cerr << "the file : [" << config.input_filename()
              << "] doesnt exist. abort." << std::endl;
    abort();
  }
  ...
  if (initial_dataset < 0) {
    std::cerr << "the dataset : [" << config.input_dataset()
              << "] doesnt exist. abort." << std::endl;
    abort();
  }
  ...
  herr_t err_read = H5Dread(initial_dataset, H5T_NATIVE_DOUBLE, memspace_id,
                            fspace_id, plist_read_id, ptr);
