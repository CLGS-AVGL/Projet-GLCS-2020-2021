...
void warning(const std::string &to_print, const int rank) {
  if (rank == 0)
    std::cerr << std::endl << "Warning: " << to_print << std::endl << std::endl;
}
...
void ConfigReader::init_options(po::options_description &options) const {
  ...
  options.add_options()("N-iter", po::value<int>()->default_value(10),
                        "number of iterations (default 10)")(
      "height", po::value<int>()->default_value(4),
      "height of the domain (default 4)")("width",
...
void ConfigReader::set_config_file(const std::string &filename,
  ...
  boost::optional<std::string> in_dataset_opt =
      tree.get_optional<std::string>("input-dataset");
  if (in_dataset_opt)
...
// the processus of rank 0 print the current configuration on the standard
// output
void ConfigReader::print_config(const po::variables_map &vm,
                                const int rank) const {
  if (vm.count("print-args") && rank == 0) {
    ...
    if (!m_input_dataset.empty())
      std::cout << "m_input_dataset = " << m_input_dataset << std::endl;
    ...
    std::cout << "m_mean_backup_interval = " << m_mean_backup_interval
              << std::endl;
    std::cout << "*******************************************" << std::endl;
    std::cout << std::endl;
  }
...
void ConfigReader::check_config(const int rank, const int size) const {
  ...
  if (m_nb_iter < 0)
    alert("The number of iterations must be positive or zero, is " +
              std::to_string(m_nb_iter),
          error_code, rank);

  if (m_global_shape[DY] < 0)
    alert("The field height must be positive or zero, is " +
              std::to_string(m_global_shape[DY]),
          error_code, rank);

  if (m_global_shape[DX] < 0)
    alert("The field width must be positive or zero, is " +
              std::to_string(m_global_shape[DX]),
          error_code, rank);

  if (m_dist_extents[DY] < 0)
    alert("The number of processus on the y axis must be positive, is " +
              std::to_string(m_dist_extents[DY]),
          error_code, rank);

  if (m_dist_extents[DX] < 0)
    alert("The number of processus on the x axis must be positive, is " +
              std::to_string(m_dist_extents[DY]),
          error_code, rank);

  if (m_dist_extents[DX] * m_dist_extents[DY] > size)
    alert("Not enough processus, " + std::to_string(m_dist_extents[DX]) + "*" +
              std::to_string(m_dist_extents[DY]) + "!=" + std::to_string(size),
          error_code, rank);

  if (m_delta_t <= 0)
    alert("The delta t must be positive, is " + std::to_string(m_delta_t),
          error_code, rank);

  if (m_delta_space[DY] <= 0)
    alert("The distance between points on the y axis must be positive, is " +
              std::to_string(m_delta_space[DY]),
          error_code, rank);

  if (m_delta_space[DX] <= 0)
    alert("The distance between points on the x axis must be positive, is " +
              std::to_string(m_delta_space[DX]),
          error_code, rank);

  if (m_backup_interval < 1)
    alert("The full backup interval must be superior or equal to 1, is " +
              std::to_string(m_backup_interval),
          error_code, rank);

  if (m_mean_backup_interval < 1)
    alert("The mean backup interval must be superior or equal to 1, is " +
              std::to_string(m_mean_backup_interval),
          error_code, rank);
  ...
  if (!m_output_filename.empty() &&
    ...
    alert("The given output file already exist, please remove it or change the "
          "filename (don't forget the mean file)",
          error_code, rank);

  if (m_output_filename.empty())
    warning("Without an output file the mean mean value backup is disabled",
            rank);
